...
func add_persistent_value( array: Array) -> void:
	if check_persistent_value( array ) == false:
		current_save.persistence.append(array)
	pass


func check_persistent_value( array: Array ) -> bool:
	var p = current_save.persistence as Array
	if p.size() == 0:
		return false
	return p[0].has( array[0])
	
func get_position_value( _array:Array):
	var p = current_save.persistence as Array
	print("Chest at ", p[0][1])
	return Vector2(p[0][1], p[0][2])
