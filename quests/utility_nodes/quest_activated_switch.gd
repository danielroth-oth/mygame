...
@export var react_to_gloabal_signal : bool = false
...
func _ready() -> void:
	if Engine.is_editor_hint():
		return
	$Sprite2D.queue_free()
	if react_to_gloabal_signal == true:
...
func  update_summary() -> void:
	...
	if check_type == CheckType.HAS_QUEST:
		...
		setting_summary += "Checking if player has completed step :" + get_step()
	elif check_type == CheckType.ON_CURRENT_QUEST_STEP:
		setting_summary += "Chek if player is on step: " + get_step()
	elif check_type == CheckType.QUEST_COMPLETE:
		setting_summary += "Cheking if quest is complete"
	pass
